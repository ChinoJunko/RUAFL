...
//                         addToSources(snk);
//                         addSrcToEdge(snk, new CallDirSVFGEdge(const_cast<VFGNode*>(snk), nullptr, getSVFG()->getCallSiteID(it->first, fun)));
...
// bool UseAfterFreeChecker::runOnModule(SVFIR* pag) {
//     initialize(pag->getModule());
...
//         DBOUT(DSaber, outs() << "Start.... " << Src->getId() << "\n");
...
//     DBOUT(DSaber, outs() << "Visiting[b] " << getSVFGNodeMsg(CurrNode)<< "\n");
...
//                 getSVFG()->getCallSite(XCSID)->getCallSite(), true);
//         pop();
//     }
...
//         if (Ancestor == CurrNode || !Ancestor->getICFGNode()->getBB())
//             continue;
...
//         std::vector<const SVFGEdge*> Ctx,const SVFInstruction* CS, bool TagX) {
...
//         auto* Inst = dyn_cast<llvm::Instruction>(PAGE->getInst());
//         if(TagX && Inst && !Inst->getType()->isVoidTy())
//             for (auto It = Inst->user_begin(), E = Inst->user_end(); It != E; ++It) {
//                 auto User = llvm::dyn_cast<llvm::Instruction>(*It);
//                 if (!User)
//                     continue;

//                 bool report = false;
//                 if (auto* X = SVFUtil::dyn_cast<LoadStmt>(User)) {
//                     report = X->getPointerOperand() == Inst;
//                 } else if (auto* X = llvm::dyn_cast<llvm::StoreInst>(User)) {
//                     report = X->getPointerOperand() == Inst;
//                 } else if (auto* X = llvm::dyn_cast<llvm::CallInst>(User)) {
...
// void UseAfterFreeChecker::reportBug(const SVFInstruction* TailInst) {
...
// bool UseAfterFreeChecker::reachable(const SVFInstruction* From, const SVFInstruction* To) {
...
// bool UseAfterFreeChecker::check(const SVFInstruction* User) {
//     if (Nocheck.getValue()) {
//         return true;
//     }
